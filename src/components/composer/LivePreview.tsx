...
// jules edit: Import splitTextIntoSlides to render live auto-generated canvas slides for Instagram posts
  ...
  return (
    <div
      className={cn(
        ...
        darkMode ? 'bg-black text-[#e7e9ea] border-[#2f3336]' : 'bg-white text-[#0f1419] border-[#cfd9de]',
      )}
    >
      {/* Header */}
      <div className='flex items-start gap-3 mb-3'>
        ...
        <div className='flex-1 min-w-0'>
          <div className='flex items-center gap-1 flex-wrap'>
            <span className={cn('font-bold text-sm', darkMode ? 'text-white' : 'text-black')}>Display Name</span>
            <svg
      ...
      {/* Engagement */}
      {showReactions && (
        <div className={cn('flex items-center justify-between pt-2 border-t mt-2', darkMode ? 'border-[#2f3336] text-[#71767b]' : 'border-[#eff3f4] text-[#536471]')}>
          <div
            onClick={handleReply}
...
function InstagramPreview({
  ...
  // jules edit: Compute auto text-to-canvas slides when no media is present
  ...
  return (
    <div
      className={cn(
        ...
        darkMode ? 'bg-black text-[#f5f5f5] border-neutral-900' : 'bg-white text-[#1c1c1e] border-neutral-200',
      )}
    >
      {/* Header */}
      ...
      <div
        className={cn(
          ...
          darkMode ? 'border-neutral-900 bg-neutral-950' : 'border-gray-100 bg-neutral-50',
        )}
      >
        {imageCount > 0 ? (
          <img
          ...
          /* jules edit: Render the active auto-generated Instagram slide on the aspect-square canvas */
          <div
...
function FacebookPreview({
  ...
  return (
    <div
      className={cn(
        'rounded-2xl overflow-hidden font-[system-ui] max-w-full border transition-colors duration-300',
        darkMode ? 'bg-[#242526] text-[#e4e6eb] border-neutral-800' : 'bg-white text-[#1c1e21] border-neutral-200 shadow-sm',
      )}
    >
      {/* Header */}
      ...
      {showReactions && (
        <>
          <div className={cn('flex items-center justify-between px-3 py-2 text-xs border-t', darkMode ? 'border-neutral-800 text-[#b0b3b8]' : 'border-gray-100 text-[#65676B]')}>
          ...
          <div className={cn('flex border-t', darkMode ? 'border-neutral-800' : 'border-gray-100')}>
            <button
              onClick={handleLike}
              className={cn(
                'flex-1 flex items-center justify-center gap-1.5 py-2.5 text-xs font-semibold hover:bg-white/5 transition-colors',
                liked ? 'text-[#1877F2]' : (darkMode ? 'text-[#b0b3b8]' : 'text-[#65676B]'),
              )}
            >
              <ThumbsUp className='h-4 w-4' />
              Like
            </button>
            <button className={cn('flex-1 flex items-center justify-center gap-1.5 py-2.5 text-xs font-semibold hover:bg-white/5 transition-colors', darkMode ? 'text-[#b0b3b8]' : 'text-[#65676B]')}>
...
function LinkedInPreview({
  ...
  return (
    <div
      className={cn(
        'rounded-2xl overflow-hidden font-[system-ui] max-w-full border transition-colors duration-300',
        darkMode ? 'bg-[#1d2226] text-[#e0e0e0] border-neutral-800' : 'bg-white text-black border-neutral-200 shadow-sm',
      )}
    >
      {/* Header */}
      ...
      {showReactions && (
        <>
          <div className={cn('flex items-center justify-between px-4 py-2 text-xs border-t', darkMode ? 'border-neutral-800 text-[#b0b3b8]' : 'border-gray-100 text-[#666666]')}>
            <div className='flex items-center gap-1'>
              <span className='text-base'>👍❤️💡</span>
              <span>{formatCount(likes)}</span>
            </div>
          ...
          <div className={cn('grid grid-cols-4 border-t', darkMode ? 'border-neutral-800' : 'border-gray-100')}>
            <button
              onClick={handleLike}
              className={cn(
                'flex flex-col items-center gap-1 py-2.5 text-[11px] font-semibold hover:bg-white/5 transition-colors',
                liked ? 'text-[#0A66C2]' : (darkMode ? 'text-[#b0b3b8]' : 'text-[#666666]'),
              )}
            >
              <ThumbsUp className='h-4 w-4' />
              Like
            </button>
            <button className={cn('flex flex-col items-center gap-1 py-2.5 text-[11px] font-semibold hover:bg-white/5 transition-colors', darkMode ? 'text-[#b0b3b8]' : 'text-[#666666]')}>
              <MessageSquare className='h-4 w-4' />
              Comment
            </button>
            <button className={cn('flex flex-col items-center gap-1 py-2.5 text-[11px] font-semibold hover:bg-white/5 transition-colors', darkMode ? 'text-[#b0b3b8]' : 'text-[#666666]')}>
...
const PLATFORM_PREVIEWS: Record<
  Platform,
  React.ComponentType<{ content: string; showReactions: boolean; darkMode: boolean }>
> = {
  x: XPreview,
    ...
    <div className='space-y-4'>
      ...
      <div className='flex items-center justify-between border-b border-white/5 pb-2'>
        ...
        >
          {darkMode ? (
            <>
              <Sun className='h-3.5 w-3.5 text-yellow-400' /> Light Mode
            </>
          ) : (
            <>
              <Moon className='h-3.5 w-3.5 text-sky-400' /> Dark Mode
            </>
          )}
        </button>
      </div>
