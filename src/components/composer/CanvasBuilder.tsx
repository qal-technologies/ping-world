...
import { Type, Palette, Plus, Trash2, Download, Layers, Sliders, ToggleLeft, ToggleRight, Circle, Square } from 'lucide-react';
...
export function CanvasBuilder() {
  ...
  useEffect(() => {
    if (selectedOverlayIdx !== null && state.canvasTextOverlays[selectedOverlayIdx]) {
  ...
  // jules edit: Realtime debounce-free updates immediately when input modifications are made
  const handleUpdateActiveLayer = (updates: Partial<typeof state.canvasTextOverlays[0]>) => {
  ...
  const addShapeOverlay = (type: 'circle' | 'square') => {
    ...
    toast.success(`${type === 'circle' ? 'Circle' : 'Square'} added to canvas!`);
  };
  ...
  const handleMouseMove = (e: React.MouseEvent) => {
    ...
    if (draggingTextIdx !== null) {
      ...
      const targetY = Math.max(0, Math.min(100, mouseY - dragOffset.y));
      setShapes((prev) =>
        prev.map((s) => (s.id === draggingShapeId ? { ...s, x: targetX, y: targetY } : s)),
      );
    }
  };
  ...
  return (
    <div className='space-y-5 select-none'>
      <div className='flex items-center gap-2'>
        ...
        <span className='text-[10px] text-pw-muted ml-auto'>Interactive vector canvas</span>
      </div>
      ...
      <div className='grid grid-cols-1 lg:grid-cols-2 gap-4'>
            ...
            {shapes.map((shape) => (
              <div
                ...
                className={cn(
                  ...
                  selectedShapeId === shape.id && 'ring-2 ring-white/70 scale-105 shadow-xl',
                )}
              />
            ))}
            ...
            {state.canvasTextOverlays.map((overlay, i) => (
              <div
                ...
                className={cn(
                  ...
                  selectedOverlayIdx === i && 'ring-1 ring-white/50 rounded px-1 scale-105',
                )}
                style={{
                  top: `${overlay.y}%`,
                  left: `${overlay.x}%`,
          ...
          <div className='space-y-2 mt-3 w-full'>
            ...
            <div className='flex gap-2 flex-wrap'>
              {BG_PRESETS.map((preset) => (
                <button
                  key={preset.label}
                  onClick={() => dispatch({ type: 'SET_CANVAS_BG', payload: preset.value })}
                  className={cn(
        ...
        <div className='w-full relative flex flex-col items-center space-y-4'>
            ...
            <div className='p-4 w-full rounded-xl bg-white/[0.03] border border-white/10 space-y-3'>
              <div className='flex items-center justify-between'>
                <span className='text-[10px] font-bold uppercase text-pw-secondary tracking-widest'>
                  Editing Shape ({selectedShape.type})
                </span>
                <button
              ...
              <div className='flex items-center gap-3 justify-between'>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setShapes((prev) =>
                      prev.map((s) => (s.id === selectedShapeId ? { ...s, color: e.target.value } : s)),
                    )
                  }
              ...
              <div className='space-y-1.5'>
                <div className='flex items-center justify-between'>
                  ...
                  <span className='text-[10px] font-mono text-pw-muted'>{selectedShape.size}px</span>
                </div>
                <input
                  type='range'
                  min={10}
                  ...
                  onChange={(e) =>
                    setShapes((prev) =>
                      prev.map((s) => (s.id === selectedShapeId ? { ...s, size: parseInt(e.target.value) } : s)),
                    )
                  }
              ...
              <div className='space-y-1.5'>
                <div className='flex items-center justify-between'>
                  ...
                  <span className='text-[10px] font-mono text-pw-muted'>{Math.round(selectedShape.opacity * 100)}%</span>
                </div>
                <input
                  type='range'
                  min={0.1}
                  ...
                  onChange={(e) =>
                    setShapes((prev) =>
                      prev.map((s) => (s.id === selectedShapeId ? { ...s, opacity: parseFloat(e.target.value) } : s)),
                    )
                  }
                  className='w-full accent-pw-primary cursor-pointer'
                />
              ...
              <div className='space-y-1.5'>
                <div className='flex items-center justify-between'>
                  ...
                  <span className='text-[10px] font-mono text-pw-muted'>{selectedShape.borderWidth}px</span>
                </div>
                <input
                  type='range'
                  min={0}
                  ...
                  onChange={(e) =>
                    setShapes((prev) =>
                      prev.map((s) => (s.id === selectedShapeId ? { ...s, borderWidth: parseInt(e.target.value) } : s)),
                    )
                  }
                  className='w-full accent-pw-primary cursor-pointer'
                />
          ...
          <div className='space-y-3 p-4 w-full rounded-xl bg-white/[0.03] border border-white/10'>
            <div className='flex items-center justify-between'>
              <p className='text-[10px] font-bold uppercase tracking-widest text-pw-muted'>
                {selectedOverlayIdx !== null ? 'Edit Text Layer' : 'Add Text Layer'}
              </p>
              <button
              ...
              >
                {addShadow ? <ToggleRight className='h-4 w-4 text-pw-primary' /> : <ToggleLeft className='h-4 w-4' />}
                Shadow effect
              </button>
            </div>
              ...
              <div className='flex items-center gap-2'>
                <input
                  ...
                  onChange={(e) => {
                    ...
                    handleUpdateActiveLayer({ fontSize: Number(e.target.value) });
                  }}
                  min={12}
                  max={120}
            ...
            <div className='space-y-2 w-full mt-3'>
              ...
              {state.canvasTextOverlays.map((overlay, i) => (
                <div
                  ...
                  onClick={() => setSelectedOverlayIdx(selectedOverlayIdx === i ? null : i)}
                >
                  <div
                    className='h-6 w-6 rounded-md shrink-0 flex items-center justify-center'
                    style={{ backgroundColor: overlay.color + '33' }}
                  >
                    <Type className='h-3 w-3' style={{ color: overlay.color }} />
                  </div>
                  <span className='text-xs text-pw-text flex-1 truncate'>{overlay.text}</span>
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
                      removeOverlay(i);
      ...
      {/* Premium Templates */}
      <PremiumGate feature={templateFeature} isPremium={state.isPremium} showPartial={false} className='h-14 mt-4'>
        <div className='p-3 h-14 flex items-center gap-2'>
          <span className='text-sm text-pw-muted'>10+ premium templates available</span>
        </div>
      </PremiumGate>
