...
function parseFilter(filterStyle: string) {
  ...
  const saturate = parseFloat(
    filterStyle.match(/saturate\(([\d.]+)\)/)?.[1] ?? '1',
  );
  const grayscale = parseFloat(
    filterStyle.match(/grayscale\(([\d.]+)\)/)?.[1] ?? '0',
  );
  const sepia = parseFloat(filterStyle.match(/sepia\(([\d.]+)\)/)?.[1] ?? '0');
  const blur = parseFloat(filterStyle.match(/blur\(([\d.]+)px\)/)?.[1] ?? '0');
  const invert = parseFloat(
    filterStyle.match(/invert\(([\d.]+)\)/)?.[1] ?? '0',
  );
  const hue = parseFloat(
    filterStyle.match(/hue-rotate\(([\d.]+)deg\)/)?.[1] ?? '0',
  );
      ...
      for (const file of fileList) {
        ...
        try {
          ...
          const asset: MediaAsset = {
            ...
            altText: '',
            filterStyle:
              'brightness(1) contrast(1) saturate(1) grayscale(0) sepia(0) blur(0px) invert(0) hue-rotate(0deg)',
            rotation: 0,
          };
      ...
      {canUploadMore ?
        <div
          ...
          <p className='text-[10px] text-pw-muted/60 mt-1'>
            {state.isPremium ?
              `Images & videos • Up to ${maxImages} files`
        ...
        {selectedAsset ?
          <motion.div
            ...
            <p className='text-[10px] font-bold uppercase tracking-widest text-pw-muted flex items-center gap-1.5'>
              <Sliders className='h-3.5 w-3.5 text-pw-primary' /> Edit{' '}
              {selectedAsset.type === 'image' ? 'Image' : 'Video'} Filters
            </p>
            ...
            <div className='space-y-1.5'>
              ...
              <input
                ...
                onChange={(e) =>
                  updateFilter('grayscale', parseFloat(e.target.value))
                }
                className='w-full accent-pw-primary cursor-pointer'
              />
            </div>
            ...
            <div className='space-y-1.5'>
              ...
              <input
                ...
                value={filters.sepia}
                onChange={(e) =>
                  updateFilter('sepia', parseFloat(e.target.value))
                }
                className='w-full accent-pw-primary cursor-pointer'
              />
            </div>
            ...
            <div className='space-y-1.5'>
              ...
              <input
                ...
                value={filters.blur}
                onChange={(e) =>
                  updateFilter('blur', parseFloat(e.target.value))
                }
                className='w-full accent-pw-primary cursor-pointer'
              />
            </div>
            ...
            <div className='space-y-1.5'>
              ...
              <input
                ...
                onChange={(e) =>
                  updateFilter('invert', parseFloat(e.target.value))
                }
                className='w-full accent-pw-primary cursor-pointer'
              />
            </div>
            ...
            <div className='space-y-1.5'>
              ...
              <input
                ...
                value={filters.hue}
                onChange={(e) =>
                  updateFilter('hue', parseFloat(e.target.value))
                }
                className='w-full accent-pw-primary cursor-pointer'
              />
            </div>
