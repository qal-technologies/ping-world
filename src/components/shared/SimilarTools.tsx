  ...
  return (
    <div className="mt-20 space-y-6">
      <h2 className="text-2xl font-bold font-display flex items-center gap-2">
        <Sparkle className="h-5 w-5 text-pw-primary animate-spin-slow" /> Similar Tools
      </h2>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
        {suggestedTools.map((tool) => (
          <a href={tool.href} key={tool.id} className="group">
            <Card className="card-glow p-4 flex flex-col h-full bg-[#0c0d1c] border border-white/5 hover:border-pw-primary/30 transition-all cursor-pointer">
              ...
              <p className="text-xs text-pw-muted leading-relaxed flex-1">
                {tool.description}
              </p>
            </Card>
          </a>
